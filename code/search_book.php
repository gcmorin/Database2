...
<?php
  ...
  if ($type == "All"){
	$all_book = "select * from book where isbn = (select isbn )";
  ...
  $best_seller = "select * from book where isbn = (select isbn from cart, orders where cart.order_id = orders.order_id and orders.date like '%$values%' group by isbn order by SUM(quantity) desc limit 1)";
